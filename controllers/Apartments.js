...
async function getCurrentApartment(req, res) {
    try {
        ...
        Apartment.findOne({ owner: req.user.id })
          ...
          .then(apaetment => {
            if (!apaetment) {
              errors.noprofile = 'There is no apartment for this user';
              return res.status(404).json(errors);
            }
            res.status(200).json(apaetment);
        })      
    } catch (err) {
        res.status(500).json({
            status:500,
    ...
    //update without arrays:
     ...
     async function editeApartment2(req, res) {
        try {
            ...
            if (req.body.mainImg) apartmentFileds.mainImg = req.body.mainImg;
            ...
            apartmentFileds.picturs = {};
            if (req.body.img) apartmentFileds.openHouse.img = req.body.img;
            if (req.body.title) apartmentFileds.openHouse.title = req.body.title;
    ...
    // //by apartment id
     ...
     async function getApartmentByuser(req, res) {
        try {
            ...
            else return res.status(200).json(apartments);
     ...
     async function search(req, res) {
        try {
            ...
            console.log(serches)
            if (serches)
                return res.status(200).json(serches);
            if (serches === [])
